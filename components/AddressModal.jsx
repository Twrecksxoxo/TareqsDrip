...
const AddressModal = ({ setShowAddressModal }) => {
    ...
    return (
        <form onSubmit={e => toast.promise(handleSubmit(e), { loading: 'Adding Address...' })} className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md h-screen flex items-center justify-center">
            <div className="flex flex-col gap-5 text-slate-200 w-full max-w-sm mx-6 bg-gradient-to-br from-slate-800 to-slate-900 p-8 rounded-2xl border border-cyan-500/30 shadow-2xl shadow-cyan-500/20">
                <h2 className="text-3xl text-cyan-400">Add New <span className="font-bold text-transparent bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text">Address</span></h2>
            ...
            <XIcon size={30} className="absolute top-5 right-5 text-cyan-400 hover:text-cyan-200 cursor-pointer transition-colors" onClick={() => setShowAddressModal(false)} />
        </form>
    )
}
